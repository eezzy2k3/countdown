...
const createCountDown = async (req, res, next) => {
  try {
    ...
    if (checkEventName)
      return next(
        new ErrorResponse(
          `The event name ${eventName} already exist, create a unique event name`,
          400
        )
      );
    ...
    if (difference < 0)
      return next(
        new ErrorResponse(`Event ${eventName} has occurred already`, 400)
      );
    ...
    res
      ...
      .json({
        success: true,
        message: `you have successfully created ${eventName} event`,
        countdown,
      });
  } catch (error) {
